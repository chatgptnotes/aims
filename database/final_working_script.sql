-- ============================================================================
-- FINAL WORKING SCRIPT - ALL SYNTAX ERRORS FIXED
...
-- STEP 5: CREATE POLICIES (FIXED - NO IF NOT EXISTS)
-- Drop existing policies first to avoid conflicts
DROP POLICY IF EXISTS "Enable all access for authenticated users" ON public.clinics;
DROP POLICY IF EXISTS "Enable all access for authenticated users" ON public.eeg_reports;

-- Create new policies
CREATE POLICY "Enable all access for authenticated users" ON public.clinics
FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Enable all access for authenticated users" ON public.eeg_reports
FOR ALL USING (auth.role() = 'authenticated');
