/**
 * Apply Supervisor UID Migration
...
async function applyMigration() {
  console.log('🚀 Starting Supervisor UID Migration...\n');
  ...
  try {
    ...
    if (alterError && !alterError.message.includes('already exists')) {
      ...
      console.log('✅ clinic_code column added successfully');
    }
    ...
    for (const org of organizations) {
      if (!org.clinic_code) {
        ...
        if (updateError) {
          ...
          console.log(`✅ Generated code "${finalCode}" for ${org.name}`);
          updated++;
        }
      } else {
        usedCodes.add(org.clinic_code);
        console.log(`ℹ️  Organization "${org.name}" already has code: ${org.clinic_code}`);
      }
    }
    ...
    console.log('\n🎉 Supervisor UID format is now active!');
    console.log('📋 Format: CLINICCODE-YYYYMM-XXXX');
    console.log('📝 Example: HOPE-202502-0012\n');
