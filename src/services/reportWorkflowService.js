...
class ReportWorkflowService {
  ...
  async executeFileUpload(workflowId, edfFile, patientInfo, clinicId) {
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Could not save file record to database (table may not exist yet):', error.message);
      }
      ...
      try {
        ...
        console.log('✅ Immediate report entry created:', reportId);
        workflow.results.reportId = reportId; // Store for later update
      } catch (error) {
        console.error('❌ Failed to create immediate report entry:', error);
    ...
    try {
      ...
      if (reportId) {
        try {
          ...
          console.warn('⚠️ Could not update report progress:', error.message);
        }
      }
    ...
    try {
      ...
      if (reportId) {
        try {
          ...
          console.log('✅ Report updated with complete analysis:', reportId);
        } catch (error) {
          console.error('❌ Failed to update final report:', error);
          throw error;
        }
      } else {
        // Fallback: Create new report if no reportId found (shouldn't happen)
        console.warn('⚠️ No reportId found in workflow, creating new report');
        const newReport = {
  ...
  async updateWorkflowInDatabase(workflow) {
    try {
      ...
      console.log('✅ Workflow updated in database:', workflow.id);
    } catch (error) {
      // Workflow tracking is optional - continue even if database update fails
      console.warn('⚠️ Could not update workflow in database (table may not exist yet):', error.message);
    }
  }
