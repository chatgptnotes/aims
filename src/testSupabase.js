...
async function testSupabaseIntegration() {
  console.log('START: Testing Supabase Integration...\n');

  try {
    // 1. Test connection
    console.log('1️⃣ Testing connection to Supabase...');
    const clinics = await SupabaseService.get('clinics');
    console.log(`SUCCESS: Connected! Found ${clinics.length} clinics in database\n`);
    ...
    console.log('SUCCESS: Clinic added:', addedClinic.id, addedClinic.name, '\n');
    ...
    console.log('SUCCESS: Found clinic:', foundClinic?.name, '\n');
    ...
    console.log('SUCCESS: Updated clinic name to:', updated.name, '\n');
    ...
    console.log('SUCCESS: Found by email:', foundByEmail?.email, '\n');

    // 6. Test delete
    console.log('6️⃣ Testing DELETE operation...');
    await SupabaseService.delete('clinics', addedClinic.id);
    console.log('SUCCESS: Clinic deleted\n');

    // 7. Verify deletion
    console.log('7️⃣ Verifying deletion...');
    const deletedClinic = await SupabaseService.findById('clinics', addedClinic.id);
    console.log('SUCCESS: Verification:', deletedClinic ? 'Still exists (ERROR)' : 'Successfully deleted', '\n');

    console.log('COMPLETE: All tests passed! Supabase integration is working correctly.');

    // Show current data stats
    console.log('\nDATA: Current Database Stats:');
    ...
    if (error.code === '42P01') {
      console.log('\nWARNING: Table does not exist in Supabase. You need to create the database schema.');
