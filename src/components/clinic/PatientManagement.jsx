    ...
    try {
      ...
      try {
        ...
        if (authError.message && authError.message.includes('already registered')) {
          ...
          return; // Stop patient creation
        } else {
          // Other auth errors - continue but warn
          toast.warning('Patient record will be created but login may not work. Please contact support.');
        }
      }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="email"
              {...register('email', {
                required: 'Email is required for patient login',
                pattern: {
          ...
          {!patient && (
            <>
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">Patient will use this password to login</p>
              </div>
