...
const SupervisorReports = ({ onUpdate, selectedClinic: superAdminSelectedClinic }) => {
  const { user } = useAuth();
    ...
    if (watchedClinic) {
      const loadClinicPatients = async () => {
        try {
          ...
          console.error('ERROR: Error loading clinic supervisors:', error);
          setSupervisors([]);
        }
      };
      loadClinicPatients();
  ...
  const loadData = async () => {
    try {
      ...
      console.log('SUPERADMIN: SuperAdmin loading ALL supervisor reports from Supabase...');

      // SuperAdmin can see ALL data from all clinics, or filter by selected clinic
      // Ensure all database calls return arrays even if they fail
      let reportsData = [];
      let clinicsData = [];
      let patientsData = [];
      ...
      try {
        reportsData = await DatabaseService.get('reports') || [];
      } catch (err) {
        console.error('ERROR: Failed to load reports:', err);
        reportsData = [];
      }
      ...
      try {
        subscriptionsData = await DatabaseService.get('subscriptions') || [];
      } catch (err) {
        console.error('ERROR: Failed to load subscriptions:', err);
        subscriptionsData = [];
      }

      // Ensure all data is actually arrays
      reportsData = Array.isArray(reportsData) ? reportsData : [];
      clinicsData = Array.isArray(clinicsData) ? clinicsData : [];
      patientsData = Array.isArray(patientsData) ? patientsData : [];
      ...
      const filteredReportsData = superAdminSelectedClinic && reportsData.length > 0
        ? reportsData.filter(report => report && report.clinicId === superAdminSelectedClinic)
        : reportsData;

      // First, fix supervisor names in the reports
      ...
      // Enhance reports with clinic and supervisor names, and extract file paths
      const enhancedReports = validatedReports.map(report => {
        const clinic = clinicsData.find(c => c && c.id === report.clinicId);
      ...
      const filteredSupervisorsData = superAdminSelectedClinic && patientsData.length > 0
        ? patientsData.filter(supervisor => supervisor && supervisor.clinicId === superAdminSelectedClinic)
        : patientsData;
      setSupervisors(filteredSupervisorsData);

      // Calculate clinic usage and load subscriptions (using Supabase data only)
      const usageMap = {};
      const subscriptionMap = {};

      clinicsData.forEach(clinic => {
        if (clinic && clinic.id) {
          const clinicReports = reportsData.filter(report => report && report.clinicId === clinic.id);
      ...
      console.error('ERROR: Critical error loading admin supervisor reports:', error);
      setError(`Failed to load data: ${error.message || 'Unknown error'}`);
      toast.error('Error loading supervisor reports data');
  ...
  // Function to fix supervisor names in reports
  const fixPatientNames = (reports, supervisors) => {
    // Safety check: ensure inputs are arrays
    if (!Array.isArray(reports)) return [];
    if (!Array.isArray(supervisors)) supervisors = [];

    return reports.map(report => {
      // Safety check: ensure report is an object
      if (!report || typeof report !== 'object') {
        return { patientName: 'Unknown Supervisor' };
      }

      // If report already has a supervisor name, use it
      if (report.patientName && report.patientName !== 'Unknown Supervisor') {
        return report;
      }

      // Try to find supervisor by ID with multiple field variations
      let supervisor = supervisors.find(p =>
        p && (p.id === report.patientId || p.id === report.patient_id)
      );

      // Get supervisor name with multiple field variations
      ...
      if (!supervisor && report.patientName) {
        supervisor = supervisors.find(p => {
      ...
      const patientName = supervisor ? getPatientName(supervisor) : (report.patientName || 'Unknown Supervisor');
  ...
  const validateReportData = (reports) => {
    // Safety check: ensure input is an array
    if (!Array.isArray(reports)) return [];

    return reports.map(report => {
      // Safety check: ensure report is an object
      if (!report || typeof report !== 'object') {
        return { fileName: 'Unknown_Report.pdf', fileType: 'application/pdf', storedInCloud: false };
      }
  ...
  const handleFileSelect = (event) => {
    ...
    if (!event || !event.target || !event.target.files) {
      console.error('ERROR: Invalid file selection event');
      return;
    }
    ...
    if (file) {
      try {
        ...
        console.error('ERROR: File validation failed:', error);
    ...
    try {
      ...
      const responseReportData = {
        ...
        reportData: {
          ...
          reportType: 'Response Report',
          description: responseData?.notes || 'Response report uploaded by Super Admin',
          fileSize: selectedFile.size,
          fileType: selectedFile.type,
          s3Key: uploadResult.key,
          s3Bucket: uploadResult.bucket,
  ...
  const filteredReports = (reports || []).filter(report => {
    ...
    const matchesPatient = !selectedSupervisor || report?.patientId === selectedSupervisor;
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center max-w-md">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Failed to Load Supervisor Reports</h3>
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex items-center justify-end gap-3 mb-6">
        <button
          onClick={() => {
            loadData();
            toast.success('Supervisor reports refreshed!');
          }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          >
            ...
            {(supervisors || [])
              ...
              .map(supervisor => (
                <option key={supervisor?.id || Math.random()} value={supervisor?.id || ''}>
                  {supervisor?.name || supervisor?.fullName || supervisor?.full_name || 'Unknown Supervisor'}
                </option>
              ))}
          </select>
          ...
          {filteredReports.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">
                {searchTerm || selectedClinic || selectedSupervisor 
      ...
      {user?.role !== 'super_admin' && (
        <SubscriptionPopup
          ...
          clinicInfo={{
            ...
            email: user?.email || 'admin@aims.com',
            phone: user?.phone || ''
          }}
        />
      )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed rounded-md transition-colors ${
              ...
              <div className="space-y-1 text-center">
                {selectedFile ? (
                  <>
                    <Cloud className="mx-auto h-12 w-12 text-[#323956]" />
                    <div className="text-sm text-[#323956]">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <button
              ...
              className="px-4 py-2 bg-[#323956] border border-transparent rounded-md text-sm font-medium text-white hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center"
            >
              {uploadingFile ? (
                <>
...
const UploadReportModal = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium text-gray-900">Upload Supervisor Report</h3>
          <button
            onClick={onClose || (() => {})}
            className="text-gray-400 hover:text-gray-600"
          >
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          ...
          <div>
            ...
            {!selectedFile && <p className="text-red-500 text-xs mt-1">Please select a file to upload to Cloud Storage</p>}
