...
import AIMSService from '../../services/aimsService';

const CarePlanGenerator = ({ patient, aimsReport, onSave, onClose }) => {
  ...
  useEffect(() => {
    if (aimsReport?.carePlan) {
      setCarePlan(aimsReport.carePlan);
      setEditedPlan(aimsReport.carePlan);
      setLoading(false);
    } else if (patient) {
      generateCarePlan();
    }
  ...
  const generateCarePlan = async () => {
    try {
      ...
      // Simulate care plan generation if no AIMS report
      ...
      const generatedPlan = await AIMSService.generateCarePlan(mockRiskAssessment, patient);
  ...
  const handleSaveCarePlan = async () => {
    try {
      ...
      const carePlanData = {
        ...
        aimsReportId: aimsReport?.reportId,
        carePlan: editedPlan,
