...
class PIDParser {
   ...
   * @returns {Object} Parsed P&ID data
   */
  static async parse(eyesOpenFile, eyesClosedFile) {
    try {
      console.log('📄 Parsing P&ID files...');
      ...
      console.log('✅ P&ID files parsed successfully');
      return combinedData;

    } catch (error) {
      console.error('❌ Error parsing P&ID files:', error);
      throw new Error(`Failed to parse P&ID files: ${error.message}`);
    }
  }
  ...
  static async parsePDF(file, condition) {
    ...
    // Use OpenAI to extract P&ID tables
    const prompt = `You are a P&ID data extraction expert. Extract the following data from this ${condition === 'EO' ? 'Eyes Open' : 'Eyes Closed'} P&ID report:
    ...
    try {
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert at extracting structured P&ID data from medical reports. Always return valid JSON.'
          },
          {
            role: 'user',
            content: prompt
