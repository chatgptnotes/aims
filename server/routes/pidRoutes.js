...
/**
 * POST /api/pid/process
 * Process P&ID files and calculate 7 brain health parameters
 */
router.post('/process', upload.fields([
  ...
  try {
    ...
    // Step 1: Parse P&ID files
    console.log('\n📊 Step 1: Parsing P&ID files...');
    const pidData = await PIDParser.parse(eyesOpenFile, eyesClosedFile);

    // Step 2: Calculate parameters
    console.log('\n🧮 Step 2: Calculating 7 brain health parameters...');
    const calculator = new AlgorithmCalculator(pidData);
    ...
    res.status(500).json({
      error: true,
      message: error.message || 'Failed to process P&ID files',
